...
const SubscribersCategoryChart = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // استخدام الـ API المركزي الموحد
        const result = await adminApi.getMemberClassifications();
        setData(Array.isArray(result) ? result : []);
      } catch (error) {
        console.error("Error fetching subscribers classification:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const topCategory = sortedData[0]?.memberClassification ?? "غير محدد";

  const chartData = {
    labels: sortedData.map(item => item.memberClassification),
    datasets: [
      {
        label: "عدد المشتركين",
  ...
  return (
    <Card className="p-5 shadow-sm" dir="rtl">
      ...
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-base font-bold text-foreground">المشتركون حسب الفئة</h3>
        {!loading && total > 0 && (
          <span className="text-xs text-muted-foreground bg-muted px-2 py-1 rounded-lg">
            {sortedData.length} فئة
          </span>
        )}
      </div>
      ...
      {!loading && total > 0 && (
        <div className="grid grid-cols-3 gap-3 mb-4">
          {[
            ...
            { label: "عدد الفئات", value: sortedData.length.toLocaleString("ar-EG") },
            { label: "الفئة الأعلى", value: topCategory },
          ].map((stat, i) => (
      ...
      {/* Chart Container */}
      <div style={{ height: `${Math.max(sortedData.length * 44, 280)}px` }}>
        {loading ? (
      ...
      {/* Legend with Progress Indicator */}
          ...
          {sortedData.map((item, i) => {
            ...
            return (
              <div key={i} className="flex items-center gap-2">
                ...
                <span className="text-xs text-foreground flex-1 truncate">{item.memberClassification}</span>
...
export default SubscribersCategoryChart;
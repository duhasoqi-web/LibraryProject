...
import BasicInfo from "@/pages/library/components/steps/BasicInfo";
import Authors from "@/pages/library/components/steps/Authors";
import Publishers from "@/pages/library/components/steps/Publishers";
import Supplier from "@/pages/library/components/steps/Supplier";
import Review from "@/pages/library/components/steps/Review";
...
deletedBy: string | null;
  ...
  publishers: PublisherDetail | null; series: SeriesDetail | null; supply: SupplyDetail | null; 
}
...
const mapDetailsToBookData = (full: BookDetailsResponse): BookData => ({
  ...
  materialTypeId: full.materialTypeId ?? null,materialTypeName: full.materialTypeName ?? null,
createdBy: full.createdBy ?? "",
updatedBy: full.updatedBy ?? null,
...
subtitles: (full.subtitles ?? []).map((s) => ({ 
  ...
  subtitleTypeName: s.subtitleTypeName ?? null,
})),
...
const handleSave = async () => {
 ...
 const handlePrint = async () => {
  ...
  @page {
  size: A4;
  margin: 0;  /* ← هاد بشيل headers المتصفح نهائياً */
}
body {
  margin: 20mm;  /* ← وهون بتحط المسافة يدوياً */
